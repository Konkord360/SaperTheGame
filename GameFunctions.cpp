...
void move(square *pointer, int rows, int columns, square **tab, int bomb_chance, bool &quit, bool &isLoaded)
{
	...
	char plate_holder, move, pointerPlate=(char)219;
...
void choice(const int rows, const int columns, square *pointer, square **tab, int &click, bool &lost, int bomb_chance, int x, int y, int &number_of_bomb, int &bombsViewed)
{
	...
	else if (lost == 0)
	{
		if (pointer->number_of_bombs != 0)
		{
			...
			pointer->plate = pointer->number_of_bombs + 48;
		}
		else
		{
...
void show_off(square **tab, int x, int y, int rows, int columns, int &click)
{
	...
	{
		tab[x][y].is_clicked = true;
		tab[x][y].plate = tab[x][y].number_of_bombs + 48;
		click++;
	}
}
...
{
	char falgPlate = (char)94, normalPlate = (char)176;
...
/*square **loadGame(square **tab, int rows, int columns)
{
	...
	if (load.good())
	{
		...
		square **tabImport = new square *[rows];

		for (int i = 0; i < rows; i++)
			tabImport[i] = new square[columns];

		for (int i = 0; i < rows; i++)
		{
			for (int j = 0; j < columns; j++)
			{
				load >> tabImport[i][j].is_bomb;
				load >> tabImport[i][j].is_clicked;
				load >> tabImport[i][j].is_safe;
				load >> tabImport[i][j].number_of_bombs;
				load >> tabImport[i][j].plate;

			}
		}
	}

	return tabImport[rows][columns];
}*/
